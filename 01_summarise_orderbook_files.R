...
existing_files <- tibble(
  files = dir("data/orderbook", full.names = TRUE),
  ticker = gsub(".*/orderbook/nasdaq_(.*)_(.*)_summary.rds", "\\1", files),
  date = gsub(".*/orderbook/nasdaq_(.*)_(.*)_summary.rds", "\\2", files),
...
missing_files <- anti_join(existing_files, processed_files, by = c("ticker", "date")) %>%
...
orderbook_summaries <- orderbook %>%
  transmute(ts,
    ...
    signed_volume = if_else(is.na(signed_volume), 0, signed_volume),
    trading_volume = if_else(type == 4 | type == 5, m_price * m_size, as.double(NA)),
    depth0_bid = bid_size_1,
    depth0_ask = ask_size_1,
    depth5_bid = orderbook %>% compute_depth(side = "bid", bp = 5),
    depth5_ask = orderbook %>% compute_depth(side = "ask", bp = 5),
    depth50_bid = orderbook %>% compute_depth(side = "bid", bp = 50),
    depth50_ask = orderbook %>% compute_depth(side = "ask", bp = 50),
...
full_grid <- tibble(ts_minute = seq(
  from = as.POSIXct(paste0(date, "09:30:00"), tz = "GMT"),
  to = as.POSIXct(paste0(date, "15:55:00"), tz = "GMT"),
  "5 min"
))
...
orderbook_summaries <- left_join(full_grid,
  ...
  fill(midquote, depth50_bid, depth50_ask, depth0_bid, depth0_ask, depth5_bid, depth5_ask, spread) %>% # stale variables
  mutate(across(c(n, signed_volume, trading_volume, n_trades), ~ replace(., is.na(.), 0)), # set to 0 if no observation
...
# Store output ----
store_output <- paste0("data/processed_orderbook/", ticker, "_", date, "_processed.rds")
