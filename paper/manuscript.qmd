...
author: 
  - name: |
      Luis Gruber, Gregor Kastner, Stefan Voigt, and Patrick Weiss\thanks{
...
keywords: Liquidity, variance risk premium, intraday dynamics, big data, exchange-traded funds.
...
format:
  pdf:
    ...
    include-before-body:
     - text: |
        {\small\noindent\textbf{Keywords:} {{< meta keywords >}}\\
         \noindent\textbf{JEL Classification:} {{< meta jel-codes >}}}
        \clearpage
        \setcounter{page}{1}
execute:
  echo: false
...
Investors, regulators, and the media carefully monitor the *VIX* index, often referred to as a fear gauge [@carr17]. The VIX is an estimate of the option-implied variance of the S&P 500 portfolio return. Since it is computed based on option prices it reflects the (estimated) market's risk-neutral expectation of the S&P 500's future realized variance. 
As such, the *VIX*aggregates information about the market participant's expectation *and* valuation of future risks.
The first component, the expected realized variance (*ERV*), predicts future return variation. 
...
The resulting wedge is commonly referred to as the variance risk premium (*VRP*).
...
A shock in the *VRP* indicates higher perceived crash-risk probabilities [see, e.g., @Bollerslev.2013 or @bollerslevtodorovxu15] or, more generally, an increase in the marginal investors' risk aversion [@Bekaert.2009]. 
In response, the marginal investor may require higher compensation to hold the asset [@campbellgrossmanwang93, @Campbell.1999, @Martin.2017]. 
A potential equilibrium induces a flight to safety by disposing of risky assets [@longstaff04, @beberbrandtkavajecz09, @Adrian.2019, @Baele.2019].

A *VIX* shock may also indicate an *ERV* shock. 
...
As a response, a *VIX* shock due to news manifesting in higher expected realized variance may render markets illiquid [@nagel12, @drechsler.2022].
...
# The wedge between expected realized variance under $\mathbbm{P}$ and $\mathbbm{Q}$ \label{sec:example}
