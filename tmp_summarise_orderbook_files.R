...
lobster_data <- arrow::open_dataset("lobster_database", partition = "ticker")

observations <- lobster_data |> distinct(ticker, date) |> collect()

observations <- observations |> slice(1)

date <- observations |> pull(date)
ticker <- observations |> pull(ticker)

orderbook <- lobster_data |>
  inner_join(observations, join_by(ticker, date)) |>
  collect()
...
orderbook_processed <- orderbook |>
  ...
  mutate(
    ts_latency = as.numeric(lead(ts, 1) - ts), # time to next message
    ts_latency = replace_na(ts_latency, 0),
    ts_minute = ceiling_date(ts, "1 minute")
  ) |> # messages from 10:30:00 - 10:30:59.xx belong into 10:31:00
  group_by(ts_minute) |>
  summarise(
...
full_grid <- tibble(
  ts_minute = seq(
    from = as.POSIXct(paste0(date, "09:31:00"), tz = "GMT"),
...
store_output <- glue::glue(
  "data/lobster_database_processed/{ticker}_{date}.parquet"
)

if (!dir.exists("data/lobster_database_processed")) {
  dir.create("data/lobster_database_processed")
}
arrow::write_parquet(orderbook_summaries, store_output)
